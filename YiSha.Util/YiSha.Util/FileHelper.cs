        ...
        public async static Task<TData<string>> UploadFile(int fileModule, IFormFileCollection files)
        {
            ...
            switch (fileModule)
            {
                ...
                case (int)UploadFileType.Import:
                    ...
                    break;
                case (int)UploadFileType.WebImage:
                    if (file.Length > 5 * 1024 * 1024) // 5MB
                    {
                        obj.Message = "文件最大限制为 5MB";
                        return obj;
                    ...
                    dirModule = UploadFileType.WebImage.ToString();
                    break;
                case (int)UploadFileType.WeApp:
                    if (file.Length > 5 * 1024 * 1024) // 5MB
                    {
                        obj.Message = "文件最大限制为 5MB";
                        return obj;
                    ...
                    dirModule = UploadFileType.WeApp.ToString();
                    break;
                case (int)UploadFileType.WeAppFile:
                    if (file.Length > 5 * 1024 * 1024) // 5MB
                    {
                        obj.Message = "文件最大限制为 5MB";
                        return obj;
                    }
                    dirModule = UploadFileType.WeAppFile.ToString();
                    break;
                default:
