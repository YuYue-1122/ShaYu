...
namespace YiSha.Data.EF
{
    public class SqlServerDatabase : IDatabase
    {
        ...
        public async Task<IEnumerable<T>> FindListOrderBy<T>(Expression<Func<T, bool>> condition, Func<T, object> orderby) where T : class, new()
        {
            var list = await dbContext.Set<T>().Where(condition).ToListAsync();
            return list.OrderBy(orderby).ToList();
        }
        public async Task<IEnumerable<T>> FindTopList<T>(Expression<Func<T, bool>> condition, int top) where T : class, new()
        {
            if (top > 0)
            {
                return await dbContext.Set<T>().Where(condition).Take(top).ToListAsync();
            }
            else
            {
                return await dbContext.Set<T>().Where(condition).ToListAsync();
        ...
        public async Task<IEnumerable<T>> FindTopListOrderBy<T>(Expression<Func<T, bool>> condition, Func<T, object> orderby, int top, string sortType) where T : class, new()
        {
            var list = new List<T>();
            if (top > 0)
            {
                list = await dbContext.Set<T>().Where(condition).Take(top).ToListAsync();
            }
            else
            {
                list = await dbContext.Set<T>().Where(condition).ToListAsync();
            }
            if (!string.IsNullOrEmpty(sortType) && sortType.ToLower() == "desc")
            {
                list = list.OrderByDescending(orderby).ToList();
            }
            else
            {
                list = list.OrderBy(orderby).ToList();
        ...
        public async Task<IEnumerable<T>> FindTopList<T>(string strSql, DbParameter[] dbParameter, int top) where T : class
        {
            using (var dbConnection = dbContext.Database.GetDbConnection())
            {
                var reader = await new DbHelper(dbContext, dbConnection).ExecuteReadeAsync(CommandType.Text, strSql, dbParameter);
                if (top > 0)
                {
                    return DatabasesExtension.IDataReaderToList<T>(reader).Take(top);
                }
                else
                {
                    return DatabasesExtension.IDataReaderToList<T>(reader);
