...
namespace YiSha.Business.OrganizationManage
{
    public class DepartmentBLL
    {
        ...
        /// <returns></returns>
        public async Task<TData<string>> GetChildIds(long deptId)
        {

            TData<string> obj = new TData<string>();
            List<DepartmentEntity> childList = await departmentService.GetChildList(deptId);
            string childIds = string.Join(',', childList.Select(m => m.Id));
            obj.Data = childIds;
            obj.Tag = 1;
            return obj;
        }
