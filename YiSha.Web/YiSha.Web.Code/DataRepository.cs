    ...
    public class DataRepository : RepositoryFactory
    {
        public async Task<OperatorInfo> GetUserByToken(string token)
        {
            ...
            if (operatorInfo != null)
            {
                ...
                strSql.Append(@"SELECT PositionName FROM SysPosition WHERE Id=(SELECT TOP 1 BelongId FROM SysUserBelong
                    WHERE UserId="+ operatorInfo .UserId+ " and BelongType=1)");
                object positionName = await BaseRepository().FindObject(strSql.ToString());
                operatorInfo.PositionName = positionName.ParseToString();
                #endregion
            }
            return operatorInfo;
        }
