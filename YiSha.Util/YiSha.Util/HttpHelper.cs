...
namespace YiSha.Util
{
    ...
    public class HttpHelper
    {
        ...
        /// <param name="postDate">提交日期</param>
        /// <param name="cc">cookie容器，可以为null</param>
        /// <param name="encoding">网页编码</param>
        /// <returns>网页内容</returns>
        public static string GetHtmlString(string url, string postDate, CookieContainer cc, Encoding encoding, int timeout,bool isUseCert=false, string contentType = "application/json")
        {
            System.GC.Collect();
            ...
            try
            {
                ...
                if (isUseCert)
                {
                    string path = "D:/wwwroot/renrenzixunapi/web/wwwroot/1606662478_20210425_cert/apiclient_cert.p12";
                    //string path = "E:/一沙/guatuzuozhan/01_工程/02_开发/V1.0.1/02_代码/ConsultSystem/Backstage/YiSha.Web/YiSha.Admin.WebApi/wwwroot/apiclient_cert.p12";

                    X509Certificate2 cert = new X509Certificate2(path, GlobalContext.SystemConfig.MchId, X509KeyStorageFlags.MachineKeySet | X509KeyStorageFlags.PersistKeySet | X509KeyStorageFlags.Exportable);
                    request.ClientCertificates.Add(cert);
                }
                //获取或设置请求将跟随的重定向的最大数目,默认为50       
                string lastmodifiedtime = request.Headers.Get("If-Modified-Since");
                if (postDate != null)
                {
                    request.Method = "Post";
                    byte[] byterequest = encoding.GetBytes(postDate);
                ...
                using (response = (HttpWebResponse)request.GetResponse())
                {
                    ...
                    {
                        using (sr = new StreamReader(responsestream, encoding))
                        {
                            html = sr.ReadToEnd();
                        }
                    }
            ...
            {
                LogHelper.Error("请求失败" + webEx.Message);
                string msg = string.Format("Url:{0}, Status:{1}, ExMsg:{2}", url, webEx.Status, webEx.Message);
            }
            catch (Exception ex)
            {
                LogHelper.Error("请求失败" + ex.Message);
                string msg = string.Format("Url:{0}, ExMsg:{1}", url, ex.Message);
            }
            finally
            {
                if (response != null)
